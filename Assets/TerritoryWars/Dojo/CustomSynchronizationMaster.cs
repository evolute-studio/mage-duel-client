...
namespace TerritoryWars.Dojo
{
    public class CustomSynchronizationMaster : MonoBehaviour
    {
        ...
        {
            int count = await SyncConstruction(
                DojoQueries.GetQueryGameByBoardId(board_id),
                nameof(SyncGameByBoardId));
            CustomLogger.LogDojoLoop($"Synced {count} games with board id {board_id.Hex()}");
            return count;
        }
        ...
        public async Task<int> SyncUnionFind(FieldElement board_id)
        {
            int count = await SyncConstruction(DojoQueries.GetQueryUnionFind(board_id), nameof(SyncUnionFind));
            CustomLogger.LogDojoLoop($"Synced {count} union find with board id {board_id}");
            return count;
        }
